...
RSpec.describe Commands::AddFabForward do
  ...
  before { ENV['MATTERMOST_TOKEN_ADDFABFORWARD'] = 'valid_token' }
  ...
  describe ".response" do
    ...
    context "with valid token" do
      ...
      it "creates a Fab note for the user" do
        ...
        expect(user.fabs.last.forward.pluck(:body)).to include(body)
      end
      ...
      context "when the user already has a fab" do
        ...
        it "edits a note for the existing fab" do
          expect { response_body }.to change(
            user.fabs.last.forward.where(body: nil), :count
          ).by(-1)
          expect(user.fabs.last.forward.pluck(:body)).to include(body)
        end
        ...
        context "and there are no blank notes" do
          ...
          it "idk, what does it do?" do
            expect { response_body }.to change(fab.forward, :count).by(1)
            expect(fab.forward.pluck(:body)).to include(body)
          end
        end
      end
